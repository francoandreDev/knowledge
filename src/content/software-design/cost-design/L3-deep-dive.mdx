...
L2 showed the coupled and decoupled `OrderService` designs shipping the
identical feature, with no way to tell them apart except by inspecting the
code. L3 gives them a real second requirement and measures, in actual lines
touched and actual compounding cost, what that invisible difference turns
into.

<LevelIntro
  items={[
    "Extending both designs with a real requirement",
    "What changed vs. what stayed untouched",
    "Modeling the compounding cost curve in code",
  ]}
/>
...
<Checkpoint question="If a fourth notification channel gets added to the coupled OrderService the same way SMS was, does the method's cost to review and test grow by the same amount SMS added, or by more?">

By more. Each new branch doesn't just add its own logic — it adds a new interaction with every branch already there, since a reviewer now has to reason about whether the new channel's business rule (its own `if`) could interact with the existing ones sharing the same method, and a test suite has to cover the growing set of combinations. This is exactly the mechanism behind the "poorly designed" line curving upward in L2's cost chart rather than staying flat: it isn't that each individual channel gets harder to write, it's that each one lands in a method that's already more entangled than the last one it was added to.
