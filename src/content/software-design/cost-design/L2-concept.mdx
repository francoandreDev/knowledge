...
<LevelIntro
  items={[
    ...
    "Coupling: entanglement between pieces",
...
<Checkpoint question="A third team, Team C, writes OrderService so it depends on a stable .notify(order) interface, but that interface is implemented by a single EmailNotifier class that itself directly imports and calls into three other unrelated internal services. Has Team C actually achieved low coupling?">

Between `OrderService` and the notifier, yes — `OrderService` only knows about `.notify()`, exactly like Team B. But coupling is a property of every relationship in the system, not just the one you happened to check. `EmailNotifier` reaching directly into three unrelated internal services is the same high-coupling mistake Team A made, just one layer deeper — and a future change to any of those three services can still ripple outward, it just has to pass through `EmailNotifier` first instead of `OrderService` directly. A narrow interface at one boundary doesn't say anything about the boundaries behind it.
...
<Checkpoint question="A developer splits OrderProcessor into three files — PricingCalculator, OrderNotifier, AnalyticsLogger — but each new class still imports and calls the other two directly to share state. Did this split actually raise cohesion?">

The split raised cohesion inside each class (each one now has a single reason to change), but it didn't come for free — reaching across the new files to share state has re-introduced coupling between them, of the same kind Team A had with `EmailService`. Cohesion and coupling aren't opposites you can trade one for the other; they're two separate measurements of the same codebase, and a real improvement usually needs both: split by reason to change, then let the split pieces talk through narrow interfaces rather than reaching into each other directly.
