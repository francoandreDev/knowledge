...
L1 showed what a generic name like `flag` costs once two teams load it
with contradictory meanings. L2 works out the actual mechanics behind
that failure — what makes a name "intention-revealing" in the first
place, why negation makes ambiguity worse instead of neutral, and why
extracting code into a named function documents it more reliably than a
comment does.
...
<LevelIntro
  items={[
    ...
    "Extraction over comments as documentation that can't drift",
  ]}
/>
...
<Checkpoint question="Why does order.isOverdue pass the intention-revealing test while order.flag doesn't, given that both are equally short and both are syntactically valid?">

It isn't length or validity that decides this — it's whether the name
itself answers the question a reader will ask when they encounter it.
`order.isOverdue` states its own meaning directly: seeing `true` tells
you exactly what's true, with no further lookup required. `order.flag`
answers a different, useless question — "is there a flag here" — and
leaves the actual meaning of that flag entirely outside the name,
recoverable only by reading the code that set it or asking whoever wrote
it. Two names of identical length can therefore carry wildly different
amounts of information, because the test isn't about character count at
all.
...
<Checkpoint question="A teammate proposes adding a comment above each step of a long function instead of extracting named functions, arguing it's faster to write. What's the concrete risk of choosing the comment over the extraction, beyond just 'comments are less tidy'?">

The concrete risk is drift, not tidiness. A comment lives in a completely
separate place from the logic it describes, so nothing enforces that the
two stay in sync — if someone later reorders the discount and tax steps,
or changes what a step actually does, the comment can be left exactly as
it was written and the code will still compile and run. The reader has no
signal that the comment is now wrong. An extracted function doesn't have
this failure mode: its name and its behavior are the same artifact, so
reordering the calls in `calculateOrderTotal` immediately shows the new
sequence in the function names themselves, with nothing left to fall out
of date.
