...
L2 established the shape of the fix in the abstract — return a new object
instead of mutating the shared one, and keep side effects isolated. L3 walks
that fix through in real code against the actual promo-code bug, then covers
what immutability costs so the trade-off isn't one-sided.

<LevelIntro
  items={[
    "The mutating bug in real code",
    "The pure fix, end to end",
    "What immutability costs at scale",
  ]}
/>
...
<Checkpoint question="cart.items is an array of line-item objects, e.g. [{ name: 'book', price: 12 }]. applyPromoCodePure's `{ ...cart, ... }` spread only copies cart's top-level properties. If code elsewhere takes the object applyPromoCodePure returns and does `result.items[0].price = 0`, is the original realCart2.items[0].price affected?">

Yes. The spread `{ ...cart }` copies `cart` one level deep — it creates a
new object with its own `total` and `discountPercent`, but the `items`
property on that new object is still the exact same array reference `cart`
holds, and each element in that array is still the same nested object
too. Mutating `result.items[0].price` reaches through that shared
reference and changes the identical object `realCart2.items[0]` points
at. This is exactly the "assuming spread deep-copies nested objects"
failure mode in the table below — the fix above is genuinely safe for
`total` and `discountPercent` because those are primitive values copied by
value, but it would need a nested spread (or a deep-clone) to make
`items` equally safe.
