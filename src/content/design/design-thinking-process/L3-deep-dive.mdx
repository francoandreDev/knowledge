...
<Scenario label="The scenario">
  <Fragment slot="facts">
    ...
    <div class="not-prose flex flex-col gap-1 text-sm text-slate-700 dark:text-slate-300">
      ...
      <div class="flex items-center gap-1.5"><span>👀</span> Shadowed 5 employees filing real expenses</div>
...
_why_, because "the UI looks old" is the easiest thing to blame when
...
You do not need to know corporate expense software to follow this case.
Track only three things: who is frustrated, what the team actually
observes, and which behavior changes after the prototype.

```mermaid
flowchart LR
    Survey["Survey: 'UI is outdated'"] --> Skip["Skip to Prototype:\nhire a design agency"]
    Survey -.->|watch first| Shadow["Empathize:\nshadow 5 employees filing expenses"]
...
<LevelIntro
  items={[
    "Walking all five stages on one real case",
    "Telling a solution-shaped Define statement from a real one",
    "Why a Wizard of Oz prototype can beat weeks of engineering",
  ]}
/>
...
The team shadowed five employees across roles — two traveling sales reps,
...
leaves the _how_ completely open. The team adopted B, and re-scoped the
...
<Checkpoint question="Suppose the team had merged Angela's category-mismatch finding into statement B instead of splitting it into its own Define statement. What would Ideate have struggled to do well with a single merged statement covering both problems?">

It would have had to search for ideas that solve two unrelated failure
mechanisms at once — reps forgetting purchase details weeks later, and
Angela's category labels not matching the card statement's labels — and
a divergent search works best when it's aimed at one precise target.
Ideas that help with same-day logging (a photo nudge, an OCR scan) don't
touch the label-mismatch problem at all, and ideas that fix the label
mismatch (matching the category list to the card statement's vocabulary)
don't touch the timing problem. A merged statement would have produced a
muddled shortlist trying to do both jobs half-well, instead of two clean
shortlists each aimed at one real problem.
...
Here, **OCR** means software that tries to read text from a photo, and a
**corporate card feed** is the stream of purchases coming from a company
credit card account. **HMW** is shorthand for "How might we...", a prompt
format that turns a Define statement into an open design question.
...
No — not before testing the _underlying_ assumption behind all three
...
<Checkpoint question="Suppose the two-week Wizard of Oz test had shown no improvement in same-day filing at all. What would that result tell the team about the OCR idea specifically — and which stage would that send them back to, Prototype or Define?">

It would tell them the underlying assumption behind all three shortlisted
ideas — that logging at the moment of purchase improves accuracy — simply
isn't true, not that this particular prototype's execution was clumsy.
Building OCR next would have spent real engineering weeks automating a
mechanism that a spreadsheet just proved doesn't help. That result sends
the team back to Define, not Prototype: the problem statement itself
("closing the gap between purchase and filing fixes accuracy") would be
the thing that turned out wrong, the same distinction L2 draws between a
Test finding that only needs a better execution versus one that means the
whole problem framing needs re-examining.
...
<div class="not-prose flex flex-col gap-2 rounded-md border border-rose-300 bg-rose-50 p-3 text-sm dark:border-rose-800 dark:bg-rose-950">
  <div class="flex items-start gap-1.5">
    <span>⚠️</span>
    <span>
      <strong>Skipping straight from complaint to Prototype.</strong> Hiring the
      design agency on day one would have shipped a modernized version of the
      exact same broken workflow.
    </span>
  </div>
  <div class="flex items-start gap-1.5">
    <span>⚠️</span>
    <span>
      <strong>A Define statement that already names the fix.</strong> Statement
      A ("more modern form") closed off every idea except font and color choices
      before Ideate even started.
    </span>
  </div>
  <div class="flex items-start gap-1.5">
    <span>⚠️</span>
    <span>
      <strong>
        Prototyping the flashiest idea instead of the cheapest test.
      </strong>{" "}
      OCR was the most exciting idea in the room and the most expensive way to
      learn whether the underlying premise even held.
    </span>
  </div>
  <div class="flex items-start gap-1.5">
    <span>⚠️</span>
    <span>
      <strong>Grading Test on opinions instead of behavior.</strong> Three of
      five volunteers said the process "felt about the same" — the timestamps
      said otherwise, and the timestamps are what the Define statement was
      actually about.
    </span>
  </div>
</div>
...
In that table, **stakeholders** are the people or groups affected by the
decision, **contractual** means promises written into agreements,
**compliance** means rules the organization must follow, and **rollout**
means releasing the change gradually to real users.
