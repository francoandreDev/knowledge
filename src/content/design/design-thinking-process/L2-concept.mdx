...
L1 laid out the five stages as a named sequence: Empathize, Define,
Ideate, Prototype, Test. L2 shows why teams that treat that sequence as a
straight line keep hitting the same bug, and works out precisely what
each stage is protecting against when it's skipped.

<LevelIntro
  items={[
    "Why Test loops back to three different stages, not one",
    "What separates real Empathize from a survey",
    "Writing a Define statement that isn't a hidden solution",
  ]}
/>
...
<Checkpoint question="A different team writes: 'Kids need a bigger menu board with better lighting so they can read it faster.' Using the fill-in-the-blank shape above, what's wrong with this statement, and how would you rewrite it so Ideate still has somewhere to go?">

It already names the fix twice over — a board, and specifically a bigger,
better-lit one — so Ideate would only be able to argue about board size
and wattage, not about other ways to help kids decide faster. A real
Define statement stays underneath the fix: "Students at the food station
need a way to decide what to take before reaching the counter, because
deciding under time pressure is what causes the stall." That version says
nothing about boards, lighting, or any other artifact, which is what
leaves room for a menu board, a sample tray, a posted daily special, or
anything else Ideate might surface.
...
<Checkpoint question="Instead of timing the line with the paper board up, the team could have asked students in the hallway afterward, 'did you like the new board?' Why is the stopwatch reading stronger evidence than that question, and what does the difference tell you about what Test is actually supposed to measure?">

The stopwatch measures the exact thing the Define statement claimed was
broken — how long it takes a student to decide before reaching the
counter — while "did you like it?" measures how the board made someone
feel about the board, which is a different fact entirely. A student can
like a board and still take just as long to decide, or dislike it and
decide faster anyway; opinion and behavior aren't the same measurement,
and only one of them can confirm or disprove the specific claim Define
made. That's the general rule Test is built on: it checks the behavior
the problem statement was actually about, not general satisfaction with
whatever got built.
