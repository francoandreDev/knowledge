...
<LevelIntro
  items={[
    "Auditing a live tree instead of guessing at it",
    "Tree tests and card sorts, and what each one actually measures",
    "Redesigning without breaking paths that already work",
  ]}
/>
...
- **Open card sort** (24 participants, no menu shown): sort 30 product
  ...
  61% _also_ put it in a "Climbing" pile — a genuine cross-listing
  signal, not noise.
- **Tree test** (60 participants, given only the existing menu labels
  ...
  no visual design shown): measures whether the _current_ tree lets
  people complete real tasks, isolated from how the page looks.

A tree test is deliberately plain: give someone a task and only the menu
tree, then watch which branch they choose. No photos, colors, hero copy,
or sale banners are allowed to rescue the structure. That is what makes
it an IA test rather than a visual-design test.

| Product        | Tree test success | Card-sort category           | Diagnosis                                                                            |
| -------------- | ----------------- | ---------------------------- | ------------------------------------------------------------------------------------ |
| Rain jacket    | 34%               | Hiking (82%), Climbing (61%) | Wrong branch _and_ needs 2 branches                                                  |
...
Tent stakes score well under the _old_ nav not because "Hardgoods" is a
...
<Checkpoint question="Tent stakes scored 88% on the tree test under the old 'Hardgoods' nav. Does that mean 'Hardgoods' is a good label, and should the redesign leave that branch untouched?">

No — the good score is a coincidence of adjacency, not evidence the
label works. Campers already land in the Hardgoods section while
shopping for a tent, so stakes get found as a nearby add-on regardless
of what the branch is called; the label itself was never tested in
isolation. Treating an 88% success rate as proof "Hardgoods" is a good
label would be reasoning backward from an outcome to a cause that isn't
actually the one producing it — the real driver is that tents and
stakes happen to sit next to each other, not that "Hardgoods" means
...
<Checkpoint question="The redesign keeps 'Shop by product type' as a secondary path instead of deleting it. Given that the tree test showed Hiking/Climbing/Camping fixes findability for the diagnosed products, why keep the old path at all?">

Because the tree test only showed the old structure failing for the
specific products that were diagnosed as broken — rain jackets and
trekking poles — not that every visitor was failing on every product.
Some visitors already think in warehouse terms and were completing
their tasks fine under the old path; deleting it to force everyone
through the new activity-based tree would fix findability for the
visitors who were struggling while breaking it for the visitors who
weren't. Keeping both paths respects that a redesign aimed at one
mismatch can create a new one if it removes a route that was already
...
Three terms in that paragraph are easy to miss:

| Term              | Plain meaning                                                                       |
| ----------------- | ----------------------------------------------------------------------------------- |
| SKU               | One sellable inventory item, often one product/size/color combination               |
| Breadcrumb        | A small path label like `Home > Hiking > Rain jackets` showing where you are        |
| Faceted filtering | Filters such as size, price, brand, waterproof rating, or color inside a large list |
