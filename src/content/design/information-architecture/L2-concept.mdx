...
<LevelIntro
  items={[
    "Hierarchy, navigation, labeling, and search/browse as one system",
    "Cross-listing: one item, more than one valid path",
    "Why fixing one part of IA can leave another part still broken",
  ]}
/>
...
<Checkpoint question="A single rain jacket product genuinely belongs under both Hiking and Climbing. Is the right fix to pick the one branch it belongs to 'most,' or something else — and why?">

Something else: cross-list it under both branches rather than forcing a
single winner. Picking one branch "most" would still leave every visitor
who arrives from the other activity unable to find it, which is the exact
failure the taxonomy exists to prevent — the product is genuinely relevant
to two different reasons someone might be shopping, so the taxonomy should
offer two real paths in rather than pretending only one is correct.
Forcing a single home doesn't simplify the tree, it just moves the
mismatch from "no path exists" to "the wrong path is the only one that
does."
...
<Checkpoint question="A visitor searches 'raincoat' on Trailhead and gets zero results, even though the site sells the exact product under the name 'shell jacket.' Is this a browse problem, a hierarchy problem, or a search problem — and would renaming the top-level nav fix it?">

It's a search problem, specifically a vocabulary mismatch between the
visitor's word and the product's word — and renaming the nav wouldn't
touch it, because the visitor never opened a menu at all; they typed a
word directly into the search box and got nothing back. Hierarchy and
labeling govern what happens when someone browses; search fails on its
own axis, when the index only matches the exact term a product is
listed under. Fixing it means teaching search to recognize "raincoat"
as a synonym for "shell jacket," not restructuring any branch of the
tree.
