...
<Scenario label="The panel">
  <Fragment slot="facts">
    <div class="not-prose flex flex-col gap-1.5 text-sm text-slate-700 dark:text-slate-300">
      ...
      <div><strong>Pass 4</strong> — alignment (confirm the shared edge)</div>
    </div>
...
<LevelIntro
  items={[
    "Four passes applied to one real panel",
    "The WCAG contrast formula worked on a real gray",
    "Where the levers create genuine trade-offs",
  ]}
/>
...
<Checkpoint question="The panel swapped #767676 (4.54:1, barely passing AA) for #333333 (12.6:1, comfortably AAA). If a designer later asks to lighten the section-label gray again for a softer look, what's the actual constraint on how far that gray can move — and why can't it be settled by eye the way the original bare-form gray was?">

The constraint is the contrast ratio against the actual background, not a
vague sense of "still readable" — any new gray has to be run back through
the same formula and land at or above 4.5:1 for normal text (or a
comfortable margin above it, given how thin #767676's pass turned out to
be). It can't be settled by eye because that's exactly how the panel ended
up with a borderline gray in the first place — "looks fine" and "measures
fine" are different claims, and only the second one is reliable across
screens, lighting, and rendering engines.

</Checkpoint>

## Pass 4 — alignment: confirm the shared edge

Alignment did not need a dramatic repair in this panel because the title,
section labels, checkboxes, and button already sit on one left edge after
the spacing pass. That is still a design decision worth naming: if the
checkbox labels started at three different x-positions, or the save
button floated under the second column, the eye would have to re-find the
reading path at every row.

The quick audit is simple:

| Element                     | Alignment question                                      |
| --------------------------- | ------------------------------------------------------- |
| Panel title and sections    | Do they begin on the same left edge?                    |
| Checkbox rows               | Do the boxes and labels repeat the same x-position?     |
| Save button                 | Does it line up with the content it acts on?            |
| Error/help text, if present | Does it sit under the field it explains, not elsewhere? |

So L3 uses all four levers: spacing and type make the grouping and
hierarchy visible, contrast makes the text reliably readable, and
alignment keeps the scan path stable.
...
<Checkpoint question="The mint-green button example computed a 1.4:1 contrast ratio against white — nowhere close to passing. Given the fill-vs-text resolution above, is there any context on the page where the literal brand mint could still be used as text color, or does the ratio rule that out everywhere?">

It rules it out everywhere the color is carrying text, regardless of size
or context — 1.4:1 fails even the loosest threshold in the WCAG table
(3:1 for large text or UI components), so there's no font-size workaround
that saves it. The literal brand mint still has a legitimate home on the
page: as a fill behind dark text, an icon, or a border, where the contrast
rule is either looser or doesn't apply to that element at all. The
distinction that matters isn't "is this the brand color" — it's "is this
pixel the thing being read," and only the second case is bound by the
ratio.
...
**Extend it yourself: this redesign assumed a light theme and a standard information density. What would Pass 3's color math need to redo for a dark theme (dark background, light text) — does the same `#767676` gray keep or lose its AA pass once the background flips from white to near-black? And separately: if this were an admin table showing 200 rows instead of two toggle groups, which of the four passes would you scale back first, and which would you keep untouched?**

The pass structure — spacing, then type, then verified color, with an
alignment audit before calling it done — isn't the only valid order, but
it's a useful default: each pass is checkable independently of the
others, and grouping/hierarchy problems are usually cheaper to fix than
contrast problems, so finding them first avoids re-verifying contrast on
a layout that's about to be restructured anyway.
