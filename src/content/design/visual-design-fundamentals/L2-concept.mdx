...
<LevelIntro
  items={[
    "Type scale as a fixed ratio, not a size list",
    "The WCAG contrast formula, step by step",
    "Why spacing and alignment are separate levers",
  ]}
/>
...
<Checkpoint question="Two grays both clear the WCAG AA threshold for normal text (4.5:1) — one at 4.54:1, the other at 9:1. Are they equally safe choices for a page's body text, given what the ratio actually measures?">

No. Both technically pass, but 4.54:1 clears the line by less than a
rounding error — a slightly different gamma-correction implementation, a
low-quality display, or a color picked from a slightly different swatch
could tip that specific pair below 4.5:1 in practice, even though the math
says it passes today. 9:1 has real margin: it would have to be badly wrong
to fail. The threshold is a minimum, not a target — a color that barely
clears it is a legitimate pass but a fragile one, and the closer the margin,
the more worth double-checking against the actual rendered background
rather than trusting the one calculation.
...
<Checkpoint question="A form already has a clean type scale (clear size jumps) and passes contrast on every label. A reviewer still says it feels disorganized. Given what alignment specifically fixes, could that complaint be real, and why wouldn't fixing type or contrast further address it?">

Yes, it could be entirely real. Type scale answers "what matters most" and
contrast answers "can I read it" — neither one says anything about where
each element's edge sits relative to the others. If labels and fields
don't share a left edge, the eye still has to re-find the start of every
line even when each line is perfectly sized and perfectly readable on its
own. Pushing harder on size or color contrast doesn't touch that problem
at all, because it's a spatial-anchoring issue, not a legibility or
hierarchy issue — it needs a shared edge, which is a different fix
entirely.
