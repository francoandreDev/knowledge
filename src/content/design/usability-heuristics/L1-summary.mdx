...
<LevelIntro
  items={[
    "Correctness vs. usability as separate axes",
    "Nielsen's 10 heuristics, grouped into 5 questions",
    "Naming which heuristic a bad interface violates",
  ]}
/>

Think of a payment machine that takes your money but prints no receipt,
shows no success message, and gives no way to cancel a mistaken choice.
Maybe the payment technically worked. You still leave unsure, cautious,
and likely to ask someone for help. Usability heuristics name that kind
...
| Term           | Plain version                                                          |
| -------------- | ---------------------------------------------------------------------- |
| Request        | A message from the page asking the system to do something              |
| Round-trip     | The request goes to the system and a response comes back               |
| Endpoint       | The specific place in the system that receives that request            |
| Server-side    | The behind-the-scenes part, not what the person sees on the screen     |
| QA             | Quality assurance: checking that the feature behaves as expected       |
| Spinner/toast  | Small feedback that says "working..." or "done"                        |
| Disabled state | A button made temporarily unavailable so the user cannot double-submit |
...
"Heuristic" just means a useful diagnostic rule, not a law. It is like a
doctor asking standard questions before naming a problem: the questions
do not replace judgment, but they keep obvious causes from being missed.
