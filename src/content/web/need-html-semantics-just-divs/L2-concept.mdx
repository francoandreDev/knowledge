...
L1 established that a `<div>` carries no meaning of its own while a semantic
tag like `<button>` or `<nav>` does — and that the gap only shows up once
someone isn't using a mouse. L2 works out precisely what that meaning
consists of: a second tree the browser builds alongside the DOM, and the
concrete behaviors a real element populates it with for free.

<LevelIntro
  items={[
    "The DOM tree vs. the accessibility tree",
    "What a real <button> gives you that a styled <div> doesn't",
    "How landmarks let assistive tech skip around a page",
  ]}
/>
...
<Checkpoint question="A team ships <div>Sign up</div> styled to look exactly like a button, and sighted QA signs off because it looks and clicks correctly. What specifically is missing from the accessibility tree, and why doesn't visual QA catch it?">

The `<div>`'s accessibility-tree entry is just "generic" — no role, no
indication it's interactive, nothing a screen reader can announce beyond the
plain text "Sign up." Visual QA can't catch this because it only checks what
the page looks like and how it responds to a mouse; the accessibility tree is
an entirely separate structure that a sighted, mouse-driven test never
touches. The div renders identically to a real button and passes every visual
check while offering assistive technology nothing to work with.
...
<Checkpoint question="A developer adds tabindex='0' to a div-based menu item so it's keyboard-focusable, and considers the accessibility work done. Using the chart above, what's still missing, and would a screen reader announce this element correctly?">

Three of the four bars are still unaddressed: there's no keydown handler, so
pressing Enter or Space does nothing even though the element can now receive
focus; there's no ARIA role, so a screen reader still announces it as plain
text rather than as something interactive; and there's no disabled-state
handling if the item can ever be disabled. Tabindex alone only makes the
element reachable by keyboard — it says nothing about what happens once it's
reached, so a screen reader user would tab to it, hear no indication it does
anything, and have no working way to activate it.
