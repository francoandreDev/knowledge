...
<LevelIntro
  items={[
    ...
    "A minimal server, no HTTP library",
...
<Checkpoint question="The raw server above uses Connection: close, so each socket only ever carries one request. Which of the four failure modes in the table does that coincidentally hide, that switching to keep-alive would immediately expose?">

Failure mode #3, not handling partial reads across multiple pipelined requests. With `Connection: close`, `buffer` only ever needs to hold one request's worth of bytes before the socket ends, so a parser that's a little sloppy about re-synchronizing on request boundaries can still happen to work. Under keep-alive, several requests can arrive back-to-back on the same socket — now the parser has to correctly find where one request's bytes end and the next one's begin every single time, or it silently corrupts every request after the first.
