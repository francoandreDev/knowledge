...
L1 named `event.target` as the exact element a click landed on, and
showed how that can differ from what a person would visually call "the
button." L2 works out the mechanism that makes this fixable: bubbling,
the distinction between `event.target` and `event.currentTarget`, and
`closest()` as the tool that resolves one into the other.

<LevelIntro
  items={[
    "How events bubble from target up through every ancestor",
    "event.target vs. event.currentTarget in a delegated listener",
    "closest() and why it enables delegation to future elements",
  ]}
/>
...
<Checkpoint question="A delegated listener is attached to document, and a click lands on a deeply nested icon three levels inside a button. Does the listener on document ever run for that click?">

Yes. Bubbling fires the event first on the exact target (the icon), then
again on each ancestor in turn — the icon's parent, that parent's
parent, and so on — all the way up to `document`. The listener on
`document` isn't skipped just because the click landed several levels
below it; it will run, and by the time it does, the event object it
receives still reports the icon as `event.target`, even though the
listener itself lives far above it in the tree.
...
<Checkpoint question="A todo app renders 5 items on page load, then adds a 6th item dynamically when the user clicks 'Add'. If the delete listener was attached once, to ul.todo-list, using closest('.delete-btn') inside it, does the 6th item's delete button work without any extra listener setup?">

Yes. The delegated listener never enumerated the 5 buttons that existed
at setup time — it only reacts to whatever click event actually arrives
at `ul.todo-list` and asks, live, whether `event.target.closest('.delete-btn')`
finds a match. Since the 6th item's delete button is still a descendant
of `ul.todo-list`, its click bubbles up to the same listener exactly like
the first 5 did, and `closest()` walks up from whatever was clicked to
find it. Nothing about adding the new element required touching the
listener at all — that's precisely what makes delegation the right fix
for a bug caused by items appearing after the listener was attached.
