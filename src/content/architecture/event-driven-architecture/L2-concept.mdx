...
<LevelIntro
  items={[
    "Pub/sub: decoupling producer from consumers via a broker",
    "Event sourcing: state as a replay of an append-only log",
...
<Checkpoint question="Shipping publishes OrderShipped and its own HTTP request returns immediately, before Notifications or Analytics have processed anything. Is that a bug, or the design working as intended?">

Working as intended. The broker accepting the publish is the point at which `Shipping`'s job is done — it never waits on any subscriber, which is exactly what lets a slow or down `Notifications` avoid blocking `Shipping`'s own request. The cost is that every subscriber's view lags by however long delivery and processing take (eventual consistency), which is why this pattern is a poor fit whenever a caller needs to act on a value that must already reflect the write — that's the same trade CQRS's read models make below.
...
Before the programming word `reduce()` matters, the idea is just
"start with an empty summary, then apply one line of history at a time":

| History line               | Summary after reading that line                           |
| -------------------------- | --------------------------------------------------------- |
| `OrderPlaced`              | Order exists, status is `placed`                          |
| `PaymentReceived`          | Same order, now paid                                      |
| `DeliveryAttempted` failed | Same order, one failed delivery attempt                   |
| `OrderShipped`             | Same order, shipped, failed-attempt count still preserved |

The code word for this repeated updating is a fold/reduce; the human
idea is just reconstructing today's truth from yesterday's diary.

<Checkpoint question="Two different services both replay the exact same OrderPlaced → PaymentReceived → OrderShipped log, independently, with no communication between them. Do they necessarily end up with the same current state?">

Yes — as long as both apply the same folding rules to the same events in the same order, they arrive at the same result, the same way two people adding up the same column of numbers get the same total. This determinism is what makes the log trustworthy as the actual source of truth rather than just a log file next to the "real" database, and it's exactly the property CQRS below relies on: a read-model projector is nothing more than a second, independent fold over the same events, shaped for a different question than the write side asks.
...
CQRS does not require event sourcing. A simpler system might write to one
normal `orders` table, then copy only the fields a dashboard needs into a
separate `orders_by_region` table. That is still "write model" versus
"read model"; event sourcing is only one way to feed the read side.
