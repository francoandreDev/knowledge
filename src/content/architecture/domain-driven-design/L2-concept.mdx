...
guarantees that *one* write either fully happens or doesn't — it says
...
*atomicity*. It doesn't know what "consistent" means for your domain —
...
in-flight orders. The aggregate boundary is drawn around *the smallest
cluster of objects that must be consistent with each other on every
single write* — not around "everything this feature happens to touch."
...
possible. Checking the invariant *inside* the same method that changes
...
| Can it change over time and stay the same thing?      | Yes (`OrderLine #7`'s quantity can change)   | No — a change produces a *new* value, not a mutation |
...
repository's job isn't fetching rows — it's guaranteeing that the *only*
...
hands that record off *after* the transaction that changed its own state
...
**Why publish *after* the save commits, instead of the instant
...
different failure mode if ignored: treating a *conformist* relationship
like a *customer-supplier* one means filing feature requests a
third-party vendor will never see; treating a genuine
*customer-supplier* relationship like *conformist* means quietly eating
...
*enforced* every day by: which repo/package the code lives in, which
