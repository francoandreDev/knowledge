...
L1 named four styles and the forces behind each one. Here's the same
checkout feature drawn all four ways, so the difference between "layered"
and "hexagonal" — or "event-driven" and "microservices" — stops being a
definition and starts being something you can see fail differently.

<LevelIntro
  items={[
    "The same feature diagrammed four ways",
    "What actually varies between the styles",
    "The one question that dominates the decision",
  ]}
/>
...
<Checkpoint question="A codebase has a folder named domain/, but that folder imports a concrete database client directly. Is this codebase hexagonal?">

No. The folder name doesn't matter — what matters is the enforced import
direction. Since the domain imports an adapter (even one wrapped in a
`domain/` folder), the dependency arrow points the wrong way, and the
`isHexagonal` check above would fail it. "Hexagonal" describes a structural
...
<Checkpoint question="Pricing and loyalty are built and owned by the same two engineers, who talk constantly. Does splitting them into two microservices buy this team independence?">

Barely any. Independence comes from decoupling deploy schedules across
different owners — here the "boundary" runs through a team that already
coordinates every day, so splitting it just adds two deploy pipelines and a
network hop without removing any real coordination cost.
