...
<LevelIntro
  items={[
    "The four recurring architectural styles",
    "What each one optimizes for and what it costs",
    "Four questions to ask before picking one",
  ]}
/>

Before the style names, anchor the vocabulary:

| Word                 | Plain meaning in this unit                                   |
| -------------------- | ------------------------------------------------------------ |
| codebase             | the body of code the team changes together                   |
| module / folder      | a code grouping inside one app                               |
| deployment           | publishing a version so it runs in production                |
| service              | a running app boundary with its own deployment               |
| database / DB        | where durable data is stored                                 |
| network call         | one running service asking another running service for work  |
| event                | a message saying something happened, like `OrderPriced`      |
| eventual consistency | data may be correct a little later, not instantly everywhere |

The simplest analogy is a small business. A layered app is one office with
departments. Hexagonal architecture keeps the core business rule at the
center and makes outside tools plug in. Event-driven work is like leaving a
ticket in an inbox so another team can handle it later. Microservices are
separate offices that can release independently, but now they must communicate
over a real channel.

An architectural style is a name for a _recurring shape_ that solves a
...
- **Hexagonal / clean architecture** — the business logic sits in the
  ...
  the domain defines. The dependency arrow always points _inward_, toward
...
- **Microservices** — the _system_ (not just the code) is split into
...
finish the sentence _"we're choosing this because our X is going to
change/grow, and this style protects against that"_ — if not, it's a
...
3. **What does "coupled" cost us if we get it wrong?** A tightly coupled
   ...
   _services_ means a coordinated multi-team deploy.
...
For deeper versions of these ideas, this project has dedicated units:
`architecture/hexagonal-clean-architecture`, `architecture/event-driven-architecture`,
`architecture/domain-driven-design`, `product-domain/bounded-contexts`,
`infra-delivery/ci-cd-pipeline-anatomy`, and `infra-delivery/docker`.
