...
in the *shape* of this code stops that; it has to be caught by discipline
...
structure makes sense. Note this version *still* has the same synchronous
loyalty call the layered version did — hexagonal fixes *where* the decision
about failure handling lives (the adapter, in one place), not *whether* a
...
function priceOrder(order, customer, promo) {
  ...
  bus.publish("OrderPriced", { orderId: order.id, customerId: order.customerId, total });
...
service is a *further* step past event-driven, not a different mechanism —
...
and publisher — the *shape* of the message-passing didn't change. What
