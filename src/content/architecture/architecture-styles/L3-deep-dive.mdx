...
<LevelIntro
  items={[
    "The same feature built four architectural ways",
    "What each version's code actually looks like",
    "Which versions violate the checkout constraint",
  ]}
/>
...
<Checkpoint question="If loyaltyClient.notify() in Version 1 takes 30 seconds to time out instead of failing instantly, what happens to priceOrder() — and therefore to checkout — during those 30 seconds?">

`priceOrder()` doesn't return until the call resolves, so it blocks for the
full 30 seconds — checkout hangs, waiting on a partner integration that has
nothing to do with pricing. Nothing in the code's _shape_ prevents this; only
adding a timeout or moving the call out of the synchronous path would.
...
<Checkpoint question="Version 4 pulls loyaltyHandler out of the same process and into its own service. Does that change whether checkout blocks when the loyalty partner is down, compared to Version 3?">

No — Version 3 already made that call non-blocking by publishing an event
and returning immediately; Version 4 is the same shape at the deployment
level, not a different failure-handling mechanism. What changes is who can
deploy it, on what schedule, and the operational cost of running a second
service — not whether checkout is protected from a slow loyalty partner.
