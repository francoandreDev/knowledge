...
<LevelIntro
  items={[
    "Why the hexagon's arrows point inward",
    "Primary vs. secondary adapters",
    "Who owns a port's shape, and the composition root",
  ]}
/>
...
<Checkpoint question="A file inside the application core adds `import { Pool } from 'pg'` just to type a local variable — nothing else about it changes. Does this file still count as part of the core in this pattern?">

No. The moment a core file imports anything from a specific adapter's technology — Postgres's `pg` package, Express, a queue client — the dependency arrow has flipped outward, regardless of how small or well-intentioned the import is. This is exactly the kind of thing worth checking mechanically later (L3 shows the actual grep-for-imports test): "resolved" here doesn't mean the rule stops mattering, it means every later section — including which adapter can be swapped for which — assumes this direction still holds.
...
<Checkpoint question="A teammate proposes changing OrderRepository.save() to accept a raw SQL WHERE-clause string, because it would make the Postgres adapter simpler to write. Should the port's signature change to match?">

No — that's the port being reshaped around one adapter's convenience instead of the core's actual need ("save an order"), which is the exact violation this section just described. Once one adapter's shape leaks into the port, every other adapter — including the in-memory one used for fast tests — has to fake understanding that adapter's technology just to compile, which is precisely what the composition root below is designed to avoid having to do.
