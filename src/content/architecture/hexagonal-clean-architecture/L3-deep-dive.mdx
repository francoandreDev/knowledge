...
<LevelIntro
  items={[
    "A port and two adapters in TypeScript",
    "Testing the core against a fake, not Postgres",
    "Trade-offs and real failure modes",
  ]}
/>

Read the code in four layers:

| Layer        | Plain meaning                                              |
| ------------ | ---------------------------------------------------------- |
| Core rule    | "Can this order be placed?"                                |
| Port         | "I need something that can save/load orders"               |
| Real adapter | "Here is how Postgres satisfies that need"                 |
| Fake adapter | "Here is a tiny test replacement satisfying the same need" |

The TypeScript uses `Promise`, `async`, interfaces, and imports. Those are
language details; the architecture test is whether the core imports only
ports and never imports Postgres, Express, or a driver.
...
<Checkpoint question="Both ports above are written entirely in plain TypeScript types (string, number, boolean, Promise) with no import from 'pg' anywhere. Was that an accident of this example, or a requirement of the pattern?">

A requirement. A port is supposed to describe what the core needs, in the core's own vocabulary — not a hint about which database will eventually answer it. If `OrderRepository.save()` took a `pg.QueryConfig` or a raw SQL string, the interface itself would already be committing to Postgres, which means every other adapter (including the in-memory one used for fast tests) would have to fake understanding Postgres just to satisfy the type.
...
<Checkpoint question="Someone later adds a debug line to place-order.ts: import { Pool } from 'pg'; console.log(new Pool()) — just to check something during local dev, planning to remove it before merging. If it merged by accident, would the grep-for-imports check above still catch it, and would that matter for the two adapters coming up next?">

Yes to both. The grep is mechanical and doesn't care about intent — any `pg` import in a core file fails it, "just for debugging" or not. It matters for what's next because the two adapters below, and the test after them, both depend on the core never having committed to a specific technology; a single leaked import is enough to make swapping `PostgresOrderRepository` for `InMemoryOrderRepository` no longer a zero-core-change operation.
...
test("rejects an empty cart", async () => {
  ...
  await expect(useCase.execute({ id: "o1", lines: [] })).rejects.toBeInstanceOf(
    EmptyCartError,
  );
});
...
test("saves a valid order", async () => {
  ...
  expect(await orders.findById("o1")).toEqual({
    id: "o1",
...
For transaction-heavy cases, connect this unit to
`systems/transactions` and `systems/race-conditions`: a port can hide
Postgres from the core, but it does not magically make multi-step writes
atomic. If one use case must save an order and charge a payment together,
the boundary needs an explicit transaction strategy such as a unit of
work, outbox, retry, or compensating action.
