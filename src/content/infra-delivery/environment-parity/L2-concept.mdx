...
<LevelIntro
  items={[
    "Why independent setups drift without anyone deciding to",
    "What 'one built artifact, run everywhere' actually changes",
    "The concrete categories of drift, and how containers close them",
  ]}
/>
...
<Checkpoint question="A teammate suggests: 'let's just write a really thorough setup script and have everyone run it on their laptop, in CI, and on the prod host — same script, same result, right?' Does that achieve parity the way the built-artifact diagram above does?">

No. A setup script still gets _run_ independently on each machine, at different times, against whatever that machine's starting state and package registry happen to look like that day — it's the same "three environments independently trying to match a specification" pattern as before, just with the specification written down instead of tribal knowledge. It can reduce drift, but it can't eliminate it the way a single built artifact can, because installation/resolution still happens three separate times instead of once. The defining property of the parity diagram above isn't "everyone follows the same instructions" — it's that dependency resolution happens exactly once, before any environment runs anything.
...
<Checkpoint question="Suppose a team pins their container's base image and dependency manifest exactly as shown, but each environment still passes a different set of environment variables into `docker run` (a different TZ, a different feature flag). Does that break the parity guarantee described above?">

No — and the distinction matters. Parity, as described here, is about the _artifact_ being identical everywhere: the OS libraries, the runtime, the dependency tree baked in at build time by `build_image`. What's explicitly passed in at `run` time — environment variables, mounted config — is deliberate, visible per-environment configuration, not accidental drift, because it's declared at the point where the environment is started, not silently inherited from whatever state a machine happened to accumulate. The difference is between "the same thing behaves differently because someone told it to" (fine, expected, reviewable) and "the same thing behaves differently because nobody noticed the machines diverged" (the actual problem parity solves).
