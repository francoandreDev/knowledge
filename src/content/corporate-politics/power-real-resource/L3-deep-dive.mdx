...
<LevelIntro
  items={[
    "Same proposal, three different outcomes",
    "What changed between attempts, line by line",
    "Failure modes, and how the pattern transfers",
  ]}
/>
...
Simple diagnosis: the idea had correctness, but no sponsor. A
postmortem is a write-up after an incident explaining what happened;
infra means infrastructure, the systems and tools other software runs
on.
...
Simple diagnosis: the idea had correctness and a sponsor, but the
framing missed what that sponsor was being measured on. A quarter is a
three-month planning period; "accountable for" means the manager will
be judged on whether that result improves.
...
Simple diagnosis: now all three are present. The proposal is correct,
it has a sponsor, and the sponsor can connect it to a real metric.
On-call means being responsible for responding when systems break; a
bottleneck is one narrow point that slows or blocks everything else.
...
<Checkpoint question="The table shows Attempt 1 never reaching the 'sponsor found' column at all, while Attempt 2 reaches it but fails on framing. If someone only saw the two outcomes — both 'shelved' — without seeing this table, could they tell the two failures apart? Why does that distinction matter for diagnosing your own stuck proposal?">

No — from the outside, both attempts produced the identical result:
silence, no funding, no staffing. The outcome alone erases the
distinction between "never reached anyone with power" and "reached
someone with power, but the pitch didn't connect to what they're
accountable for." That distinction matters enormously for diagnosis,
because the fix is completely different in each case: a missing
sponsor means the next move is finding the right person to pitch;
a framing mismatch means the next move is reframing the same pitch to
the same person around a different metric. Treating a framing failure
as if it were a missing-sponsor failure wastes the sponsor relationship
you already have by walking away from it instead of adjusting the
pitch.
...
- **Concluding from Attempt 1 or 2 that the org is dysfunctional or doesn't value good engineering.** It's a common and understandable read, but it mistakes a missing sponsor or a framing mismatch for a verdict on the idea's merit — the same idea succeeded on attempt 3 with the identical technical content, which is direct evidence the org _could_ act on it, given the right pipeline.
  Some organizations really are dysfunctional. The narrower lesson is
  that "it was shelved" alone does not prove that; first check whether
  sponsor and framing were actually present.
...
<Checkpoint question="The second failure mode above warns against fabricating a connection to a sponsor's incentives. Attempt 3's on-call framing wasn't invented — it was true of the proposal all along, just not yet foregrounded. What's the practical test for telling a legitimate reframe apart from a dishonest one?">

The test is whether the connection was already true of the proposal
before anyone chose to emphasize it, and whether it would hold up if
the sponsor asked to see the reasoning behind it. Attempt 3's
bottleneck effect was a real, pre-existing consequence of automating
the rollback — foregrounding it didn't change what the proposal
actually did, only which of its true effects got named out loud. A
dishonest reframe works the opposite way: it invents or exaggerates a
connection to the sponsor's metric that doesn't actually follow from
what the proposal does, and it falls apart the moment the sponsor
checks the claim against reality — which is exactly the kind of
scrutiny a proposal should be able to survive if the framing is
honest.
