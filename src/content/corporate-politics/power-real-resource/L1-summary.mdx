...
Plain language for the setup: a deploy is releasing a software change,
an outage is a period when the system is broken for users, and a
rollback is undoing a bad release. Funding and staffing mean the same
basic thing here: people, time, and budget actually assigned to do the
work.
...
<LevelIntro
  items={[
    "Power as a real, learnable resource",
    "The four sources of power in an org",
    "Politics as neutral coordination, not manipulation",
  ]}
/>
...
The same pattern exists outside work. In a school, having the best
idea for a class event is not enough if it never reaches the teacher
or director who can approve the room, time, and supplies. The idea may
be correct, but it still needs a path to someone who can act.
...
Everyday examples: formal authority is the person who can say "yes,
use these resources"; expertise is the person people trust because
they have been right many times; relationships are people backing you
because you have been reliable with them; information is knowing what
the decision-maker is already trying to improve.

Later in the unit, the person with relevant power is called a
**sponsor**, and the way you connect the proposal to what that person
cares about is called **framing**.
...
- **Politics, in the neutral sense this unit uses the word, is just the process of getting a group of people with different incentives to align on a decision.** It's not inherently manipulative — treating it as inherently dirty is itself a stance that cedes influence to people who don't share that squeamishness.
  Its abuses are real and visible, but the neutral mechanism is simply
  coordination: different people care about different things, and a
  decision only moves when enough of those cares line up.
