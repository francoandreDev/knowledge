---
title: "L2 — Reading hedge language and silence as data, without treating a guess as a fact"
---

import LevelIntro from "../../../components/LevelIntro.astro";
import Checkpoint from "../../../components/Checkpoint.astro";

L1 named the two most common carriers of subtext — hedge language and
silence — without yet showing how to read either one without turning a
guess into an accusation. L2 works through both in detail, plus the one
move that keeps a read honest: turning it into a question instead of a
verdict.

<LevelIntro
  items={[
    "A catalog of hedge phrases and what they usually signal",
    "The one precondition that makes silence meaningful",
    "Turning a read into a low-stakes follow-up question",
  ]}
/>
...
<Checkpoint question="A colleague responds to your proposal with 'worth exploring,' and then immediately asks 'what would the migration timeline look like if we started next quarter?' Using this level's own test for telling a real hedge from literal agreement, is 'worth exploring' functioning as a hedge here?">

No. The test given above isn't the phrase alone — it's the phrase combined
with whether a specific, substantive follow-up came with it. "Worth
exploring" paired with a concrete question about migration timeline is
exactly the kind of specific engagement that real interest looks like; the
phrase is doing polite framing, not standing in for the reaction the way it
would if nothing concrete followed it. The same words that read as a hedge
in isolation stop being one once they're followed by a specific question
...
```mermaid
flowchart TD
    A["Someone stays silent\nduring a discussion"] --> B{"Do you know how\nthis person normally\nreacts to proposals\nlike this?"}
    B -->|"No baseline known"| C["Silence is not\nmeaningful data yet —\njust an absence"]
    B -->|"Yes — they'd normally\nask hard questions\nor push back"| D["The absence of their\nusual reaction IS\nthe signal"]
    D --> E["Treat it as a prompt\nto find out more —\nnot as confirmed\nopposition"]
```
...
<Checkpoint question="A new hire who has been in exactly two meetings says nothing when your proposal comes up. Does this level's precondition for treating silence as signal apply here, and why or why not?">

No. The precondition is knowing this specific person's normal reaction, and
two meetings isn't enough to establish a baseline for anyone — you don't yet
know whether this person is naturally quiet, still finding their footing in
a new team, or genuinely someone who'd push back hard once they're
comfortable. Without that baseline, their silence is just an absence, not
evidence of anything about this proposal specifically. Reading meaning into
it here would be exactly the failure mode of treating an unestablished
pattern as if it were a broken one.
