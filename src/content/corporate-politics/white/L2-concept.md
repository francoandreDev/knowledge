---
title: "L2 — The transparency test, and three mechanisms that pass it"
---
...
```mermaid
flowchart LR
    Action["An influence-building action"] --> Test{"Would this still work,\nundamaged, if the other\nperson fully understood\nwhat you're doing and why?"}
    Test -- "Yes" --> White["White: reciprocity, visibility,\nalliance-building"]
    Test -- "No — it depends on\nthem NOT knowing" --> NotWhite["Not white — covered in\nlater units (gray/black)"]
```
...
```mermaid
xychart-beta
    title "How genuine an ask reads, by months since the last unrequested help"
    x-axis ["0-1 mo", "2-3 mo", "4-6 mo", "7-9 mo", "10-12 mo"]
    y-axis "Perceived genuineness (rough)" 0 --> 100
    bar [15, 35, 55, 75, 90]
```
...
```mermaid
flowchart LR
    Work["Good work,\ndone well"] --> Seen{"Does anyone outside\nyour immediate team\never see it?"}
    Seen -- "No" --> Skill["Builds real skill,\nzero influence beyond\nyour immediate circle"]
    Seen -- "Yes — shared deliberately" --> Trust["Builds skill AND\ntrust/influence beyond it"]
```
