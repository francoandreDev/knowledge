...
L1 introduced the two engineers and the single variable — timing and
pattern, not honesty — that explained why only one of them got spontaneous
backers. L2 works out that variable as a precise, mechanical test, and the
three legitimate mechanisms (reciprocity, visibility, alliance-building)
that pass it.

<LevelIntro
  items={[
    "The transparency test, stated precisely",
    "Reciprocity: why genuine help compounds and transactional help doesn't",
    "Visibility and alliance-building as the other two mechanisms",
  ]}
/>
...
<Checkpoint question="Someone deliberately schedules regular coffee chats with people across the org specifically because they think knowing people broadly is valuable — and they'd say exactly that, including the part about it eventually helping their own career, if asked directly. Does having that self-interested motive fail the transparency test, even though nothing about it is hidden?">

No. The test isn't "is there any self-interest here" — almost every relationship has some. The test is whether the mechanism needs the other person NOT to know what's happening in order to work. This person's coffee chats would work exactly the same, and be received exactly the same, whether or not the motive was ever stated out loud — nothing about the pattern depends on concealment. That's the opposite of a tactic that only functions because the other side doesn't see it coming. Self-interest and transparency aren't opposites; only self-interest that has to hide to function fails the test.
...
<Checkpoint question="Someone gives a colleague genuine, unprompted help for 11 straight months, then in month 12 explicitly says 'remember when I helped you with X' while making an ask. According to the table above, does that one reminder retroactively change how the previous 11 months read?">

Yes — that's exactly the third row of the table. The help itself didn't change; what changed is that calling it in immediately, right at the moment of the ask, makes the whole pattern legible as having been aimed at this ask all along, even if it genuinely wasn't. This is why the second row's mechanism depends on patience specifically: the payoff has to arrive on the other person's own initiative, not be extracted by naming the debt, or the accumulated goodwill gets retroactively recolored as a transaction that was just slow to close.
