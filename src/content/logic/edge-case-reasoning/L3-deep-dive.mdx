...
L2 built the process (partition, find boundaries, test around them) and
showed on paper why monotonicity misses the `5.0` bug. L3 reproduces that
exact bug in real code, runs the monotonicity check against it to confirm
the miss directly rather than take it on faith, and then builds the
boundary-value test suite that does catch it.

<LevelIntro
  items={[
    "Reproducing the boundary bug in real code",
    "Proving an invariant check misses it, by running it",
    "Building a boundary-value test suite that catches it",
  ]}
/>
...
<Checkpoint question="Looking at the table above, every one of the developer's three original tests (weight 2, 6, 15) passed. What single property do all three of those chosen values share that let the bug slip through unnoticed?">

All three are interior values — comfortably inside their tier, nowhere
near a tier boundary. `2` sits well inside `0–5`, `6` sits well inside
`5–10`, and `15` sits well inside `10+`. None of them is the exact
threshold value or one step away from it, so none of them can ever
distinguish `weightKg < 5` (buggy) from `weightKg <= 5` (correct) — both
versions of the function return the identical answer for all three
inputs. This is exactly L2's point made concrete: picking one
"representative" value per equivalence class tests the interior, never
the seam between classes, no matter how many interior values you add.
