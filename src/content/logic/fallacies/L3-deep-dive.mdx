...
<LevelIntro
  items={[
    "Annotating a real thread fallacy by fallacy",
    "Measuring on-topic contribution, not word count",
    "Failure modes in naming fallacies during a live disagreement",
  ]}
/>
...
<Checkpoint question="Jordan's line cites a therapist uncle's opinion that friend groups fall apart once people start skipping things. Before reading the analysis below: what specifically does that citation fail to establish about this thread's actual question?">

It fails to establish anything about _this specific group_, after _one_
missed week. A therapist's expertise on friend-group dynamics in general is
real and might be worth hearing on a genuinely open question about typical
patterns — but the thread isn't asking "do friend groups in general
sometimes fall apart," it's asking whether this particular six-person group,
after a single dip from 6 to 4, is actually declining. General authority on
a related but broader question doesn't resolve a specific, checkable claim
about this case; that gap between "expert on the category" and "correct
about this instance" is exactly what makes an appeal to authority misused
rather than simply irrelevant.
...
<Checkpoint question="Suppose someone in the thread correctly calls out Sam's straw man and restates Alex's real position. Does the disagreement then count as resolved?">

No. Correctly naming and correcting the straw man only clears away a
distortion that was blocking a real conversation — it doesn't answer the
question the group actually needs to answer, which is whether a two-person
miss is a one-off or the start of a real decline, and what a reasonable fix
would be. Removing a fallacy restores the path to that judgment call; it
doesn't make the judgment call for you. That's why the failure-mode list
above warns against treating "I named the fallacy" as a stopping point —
the fallacy-free version of the thread still ends with Alex and Casey
agreeing to check back in four weeks, not with the question already
settled.
