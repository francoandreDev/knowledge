...
L1 showed two engineers ending up with differently-reusable code from the
same feature, and traced the difference back to which one wrote an
assertion before any implementation existed. L2 works through the loop
that test-first engineer was actually running, step by step, and why each
step is there on purpose rather than just being "write tests, then code."

<LevelIntro
  items={[
    "The red-green-refactor loop, precisely",
    "Why 'minimum code to pass' is a deliberate constraint",
    "The specific design pressure writing the test first creates",
  ]}
/>
...
<Checkpoint question="A developer writes a test, runs it, and it passes immediately on the first run — before any implementation code was added. According to the loop above, is that a good sign or a red flag, and why?">

A red flag. The loop requires seeing the test fail first specifically
because a test that passes without any implementation behind it hasn't
demonstrated it's capable of failing at all — it might be asserting
something trivially true, or checking the wrong thing entirely. A test
that can't fail can't be trusted later when it passes for real, so
skipping straight to green without ever seeing red removes the one step
that proves the test is actually testing anything.
...
<Checkpoint question="A developer refactors a function that already has three passing tests, and afterward all three tests still pass without being changed. Does that prove the refactor didn't change behavior, or only that it didn't change the behavior those three tests happen to check?">

Only the second — it proves the refactor didn't break whatever those
three tests check, nothing more. Tests can only catch a regression in
behavior they actually assert on; any behavior the existing three tests
don't cover could have shifted during the refactor and nothing would
flag it. This is exactly why the refactor step's safety is relative to
test coverage, not absolute — the more of the function's real behavior
the passing tests actually pin down, the more a clean run genuinely
