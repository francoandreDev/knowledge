...
<LevelIntro
  items={[
    "The same checkout feature tested at three levels",
    "How a bug's cost changes by where it's caught",
    "Common failure modes when levels are misused",
  ]}
/>
...
<Checkpoint question="The unit tests for applyDiscount above all pass. Why can't they catch a bug where the real discount-code API starts returning percent as the string '20' instead of the number 20?">

Because applyDiscount's unit tests never touch the real API at all — they
call the function directly with hand-written numbers, so there's no
path through them for a response-shape change to travel. That kind of
bug only exists at the boundary between two real components (the
service that started returning a string, and the frontend code that
expected a number), and a unit test by design isolates the function
from exactly that boundary. Only the integration test above, which
calls the real validateDiscountCode against a real running service,
would actually receive the malformed value and fail on it — which is
the whole reason integration tests exist as a separate level rather
than treating "well, the unit tests pass" as sufficient.
...
<Checkpoint question="A team says their 'integration test suite' has 200 passing tests, but a contract mismatch between the discount service and the frontend still slipped into staging last week. Based on the failure modes above, what's the most likely explanation, and how would you check for it?">

The most likely explanation is the mocking failure mode: the "integration"
tests are probably mocking out the discount service instead of calling a
real running instance of it, which means they're really unit tests
wearing an integration test's name — they can pass forever while the
actual contract silently drifts, because nothing real is ever contacted
to notice the drift. The way to check is to look at what each test
actually calls: if `validateDiscountCode` or its equivalent is stubbed
or mocked rather than pointed at a real (even if local/ephemeral) service
instance, the suite isn't testing the boundary it claims to, no matter
how many tests it has or how green they are.
