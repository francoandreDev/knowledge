...
L1 established that the vendor meeting failed because no one had both
permission and responsibility to decide. L2 works through the two
frameworks that fix that — DACI and RACI — role by role, and how a
decision actually moves through DACI from input to outcome.

<LevelIntro
  items={[
    "The four DACI roles and what each does",
    "RACI, and how it differs from DACI",
...
<Checkpoint question="A team wants to use DACI for choosing a vendor, but decides the Approver should be 'the engineering team as a whole' so nobody feels left out. What goes wrong with that setup?">

It stops being DACI in any way that matters — the whole point of
naming an Approver is a single, named point of accountability. Making
"the team" the Approver just reintroduces the same diffusion of
responsibility that caused the original 45-minute meeting to end
without a decision: if everyone is accountable, in practice no one is,
and the group still needs something like consensus to move, which is
exactly what a real DACI Approver exists to avoid.
...
<Checkpoint question="A team lead wants to use DACI to track who's responsible for each part of a quarter-long migration project with dozens of ongoing tasks. Why is that the wrong tool, and what should they reach for instead?">

DACI is built for a single decision with a clear before/after — one
moment where an Approver makes a call. A quarter-long migration isn't
one decision; it's an ongoing matrix of tasks, each needing its own
owner over time. That's what RACI is for: assigning Responsible and
Accountable per task, with Consulted and Informed tracking who else is
involved. Reaching for DACI here would try to force an ongoing
project into a single decide-and-done shape it doesn't actually have.
