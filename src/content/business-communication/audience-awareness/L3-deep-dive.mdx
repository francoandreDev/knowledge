...
<LevelIntro
  items={[
    "The same facts, rewritten for three readers",
    "Counting operational specifics, not word count",
    "Calibration vs. deception, and the failure modes",
  ]}
/>
...
<Checkpoint question="The teacher's version is 72 words, longer than the groupmate's 62 — even though the teacher got fewer operational specifics (3 vs. 6). What did the teacher's version spend those extra words on instead?">

The trade-off and the default: explaining that the make-up sale should close the gap without pushing back the trip deadline, and offering an explicit "I'll run it as planned unless you say otherwise" so the teacher has a fast yes/no instead of an open-ended question. Word count tracks how much a version explains its reasoning and its ask — it doesn't track detail the way counting operational specifics does. A shorter message isn't automatically more audience-aware, and a longer one isn't automatically more thorough; what matters is whether the words are spent on what that specific reader needs.
...
<Checkpoint question="The 'Over-indexing on role, not stake' failure mode in the table below means giving someone the short version purely because of their title. Using the calibration/deception distinction above, is that failure a form of deception?">

No — it's a form of miscalibration, not deception. Deception means hiding or blurring a truth the reader actually needed to make their decision; over-indexing on role just means you picked the wrong lens to predict what they needed, using title as a shortcut for stake instead of checking the real question and stake directly. The fix isn't "tell them more so it's not deceptive" — the message can still be completely honest and still fail, because it was calibrated to the wrong signal. That's exactly why L2 defined the lens as the reader's actual question and stake, not their title.
