...
<Checkpoint question="Two of the four mismatches above cost you an extra round-trip (the reader has to come back and ask something). Which two, and why do the other two not cost a round-trip in the same way?">

Principal-level brevity given to the groupmate, and no clear next step given to the teacher — both leave the reader without something they need to act, so they come back and ask for it. The other two (groupmate-level detail to the principal, and vague reassurance to the principal) don't produce a round-trip in the same mechanical sense — the principal doesn't ask a clarifying question, they escalate or quietly lose trust, which is arguably worse because it doesn't give you a second chance to fix it in the same conversation.
