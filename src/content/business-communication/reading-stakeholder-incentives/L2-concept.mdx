...
L1 named the gap between the visible scorecard (cost, architecture, timeline) and the
invisible one (what saying yes or no costs a specific person) — this level turns that gap
into a concrete map you can fill in before you ever walk into the room.

<LevelIntro
  items={[
    "The four incentive-mapping questions",
    "Filling in a real stakeholder map",
    "Two levers for a second attempt: timing and framing",
  ]}
/>
...
<Checkpoint question="Of the four questions, which one alone already explains most of the team lead's resistance in the scenario, and why?">

"Loses from yes" — the flagship project losing relevance right before review. The other
three questions matter, but "measured/rewarded" and "gains from yes" are mild here (small,
indirect upside), and "timing risk" only explains why the loss is costly _right now_, not
what the loss actually is. The loss itself is the row doing the real work, which is why it
stays the single most useful column later in this level.
...
<Checkpoint question="A colleague suggests a third option: keep the original pitch exactly as-is, but just wait for a moment when the team lead is 'in a better mood.' Does that address the same row of the map as the timing and framing options above?">

Not really — it's a shallow copy of the timing lever, not a real fix. The timing option
above works because review season specifically changes the _cost_ of the loss (public
redundancy right before a career-relevant evaluation); a vague "better mood" doesn't
change what the proposal still costs the team lead once evaluated. Framing has the same
gap: without also repositioning who owns the outcome, waiting for a better mood leaves the
"loses from yes" row untouched, so the same objection is likely to resurface.
