...
<LevelIntro
  items={[
    "The same delay reported two structurally different ways",
    "Counting checkable claims, not word count",
    "Failure modes that undo good intentions",
  ]}
/>
...
<Checkpoint question="Every single sentence in Path A was arguably true when it was written. So what exactly is the reader reacting badly to, if not a lie?">

The reader isn't reacting to any one sentence — they're reacting to the pattern across three updates: the same vague, unfalsifiable phrase ("almost there," "should be wrapped up soon") repeated at each miss, with the real cause only surfacing in week 4. Each update was locally honest, but the sequence reads as either not understanding the problem or not wanting to say so, because nothing in it let the reader check the claim against reality until it had already failed twice.
...
<Checkpoint question="The underlying technical problem was identical in both paths, and Path B still finished a week earlier than Path A. If Path B's engineer wasn't a better engineer, what actually bought back that week?">

Naming the real complexity at week 2 instead of at week 4. Path A kept restating the original, too-optimistic estimate for two more cycles before finally scoping the redesign properly, which is what pushed it out to week 4. Path B absorbed the newly understood complexity into a single, larger, honest estimate immediately — so the actual engineering work had a realistic target from the start instead of being rushed against a date that was already wrong.
