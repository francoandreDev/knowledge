...
<LevelIntro
  items={[
    "Why order and specificity decide belief, not just honesty",
    "BLUF applied to bad news",
...
Quick vocabulary for the scenario:

| Term            | Plain meaning                                                        |
| --------------- | -------------------------------------------------------------------- |
| Third-party API | An outside service your system depends on                            |
| Rate limiting   | A rule that limits how often you can use that service                |
| Docs            | The service's written instructions                                   |
| Retry logic     | What your system does when it tries again after a failed attempt     |
| BLUF            | "Bottom Line Up Front": say the most important status first          |
| Falsifiable     | Specific enough that later readers can check whether it held         |
| Optimism bias   | Reporting the hopeful version as if it were already the likely truth |
...
A usable delay update can be this small:

> Status: the feature will miss today's date. Impact: one-week delay,
> no action needed from you right now. Cause: the outside API behaves
> differently than its docs. New estimate: next Friday, because the
> redesign is now scoped around the real behavior. Ask: I'll flag
