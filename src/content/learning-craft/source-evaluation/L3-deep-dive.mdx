...
<LevelIntro
  items={[
    "Tracing a claim to its primary source, step by step",
    "Calibrating how much verification a claim is worth",
    "Catching a claim stretched beyond what its source supports",
  ]}
/>
...
<Checkpoint question="A classmate mentions offhand that the cafeteria ran out of the good lunch line and is only serving the backup option today. Using the calibration question from L1 and the two examples above, is this worth tracing back to a primary source before you decide what to do?">

No — this is exactly the low-stakes, easily-reversible case where shallow trust is the right call, not a shortcut you're settling for. Being wrong costs almost nothing: you find out the moment you walk into the cafeteria, and there's no decision riding on it that can't be corrected on the spot. Spending effort tracing this back to whoever posted the cafeteria menu would be the wrong move, not the careful one — the whole point of calibrating is recognizing that some claims genuinely don't need the two-minute course-page check, let alone the fifteen-minute journal search, because the cost of being wrong is trivial and instantly self-correcting.
...
<Checkpoint question="The wellness blog in the supplement trace did technically link to something — another blog post, not the study itself. Does that link satisfy 'cites the primary source, checkably' from L2's signal ranking, or is it weaker than it looks?">

It's weaker than it looks, and the trace above is exactly why: a link only counts as citing the primary source if it actually resolves back to something checkable, and this one resolves to a second blog post that itself doesn't link the study either — it only names a journal. Two secondary layers stacked on top of each other, each citing the layer before it rather than the original paper, isn't the same as one layer citing the primary source directly. The presence of a link can make a claim feel traceable without it actually being traceable yet, which is precisely why the trace had to keep going past the first citation instead of stopping at "well, at least it links to something."
