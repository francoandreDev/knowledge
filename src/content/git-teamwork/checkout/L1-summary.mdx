...
**The command did exactly what it says — restore everything in the current directory to its last-committed state. The mistake wasn't the tool malfunctioning; it was not knowing that `checkout .` (no path, no `--`) is scoped to the whole directory, not the one file you had in mind. What single word, added before the dot, would have saved `feature.js`?**

</Scenario>

The fix (`git checkout -- config.json`, naming the file) was one word away —
the danger wasn't the command family, it was not knowing which of three
genuinely different operations you were actually invoking.
