...
L2 established that a normal push checks whether the remote's tip is an
ancestor of what's being pushed, and that force-pushing simply replaces the
ref without asking that question. L3 turns both of those into runnable
code — a small commit-history model that measures exactly which commit the
Scenario's force-push destroys, and a working `--force-with-lease` check
that would have caught it first.

<LevelIntro
  items={[
    "A minimal commit-history model with reachability",
    "Measuring exactly which commit a force-push discards",
    "Implementing the `--force-with-lease` check itself",
  ]}
/>
...
<Checkpoint question="If the developer had fetched right before rebasing, so their local history included D as C's child and E were rebased on top of D instead of C, what would commitsLostByForcePush(history, 'D', 'E') return, and why?">

It would return an empty array. `reachableHashes` walking backward from
the new local tip `E` would now pass through `D` on its way to `C`,
`B`, and `A`, so every commit reachable from the old remote tip `D`
would also be reachable from the new tip. Nothing computed as "lost"
because nothing actually would be — this is the mechanical reason
"fetch before you rewrite" fixes the problem: it's not a superstition,
it's ensuring the new tip's reachable set is a superset of the old
one's.
...
<Checkpoint question="isAncestor(history, 'D', 'E') returns false, and forceWithLease(remoteRef, 'C', 'E') also throws. Both reject the same push — so is forceWithLease just reimplementing the normal push's ancestor check under a different name?">

No — they catch the same accident here, but by checking different
things. The ancestor check (`isAncestor`) is a property of the commit
graph itself: is the remote tip reachable by walking backward from the
new tip, regardless of when anyone looked. `forceWithLease` checks a
snapshot in time: does the remote's tip right now match what the
pusher's client last saw, which is exactly the check a plain
`--force` skips on purpose so it can override history that a normal
push would reject. They agree in this example because the developer's
stale expectation (`C`) genuinely isn't the remote's real tip (`D`) —
but `forceWithLease` is the one built to run even when force is
intentional, which is the whole reason it exists as a separate flag
rather than just "always do the ancestor check."
