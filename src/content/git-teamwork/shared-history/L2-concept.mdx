...
L1 named the three copies involved in the Scenario's disappearing commit
and pointed at `--force-with-lease` as the fix, without yet showing what a
force-push actually does to the remote branch, or why a normal push would
have refused to do the same thing. L2 works through both.

<LevelIntro
  items={[
    "The ancestor check a normal push performs",
    "What a branch ref actually is, and what force-pushing moves",
    "Why the danger is stale state, not rebasing or force itself",
  ]}
/>
...
<Checkpoint question="A teammate says: 'I pushed a rebase of my own commits and it got rejected with a fetch-first error — is my repo broken?'. Using the ancestor check above, what actually happened, and what should they do next?">

Nothing is broken — the rejection is the ancestor check working exactly
as designed. Git compared the remote's current tip against the commit
they were trying to push and found the remote tip isn't an ancestor of
it, meaning the remote has at least one commit their local branch
doesn't know about yet. The fix is to fetch, look at what arrived, and
reconcile their rebased work with it before pushing again — not to add
`--force` to push past a check that just caught something real.
...
<Checkpoint question="If a force-push moves the branch ref but doesn't delete any commit objects, why does the Scenario still call the teammate's commit 'gone', and does that framing hold up once you know a ref is just a pointer?">

The framing holds up for anyone who only has access to the remote
branch, which is nearly everyone downstream of it — CI, other
teammates cloning fresh, and the branch's own commit log. The commit
object may still exist in the repository's storage for a while, but
nothing on the branch points to it anymore, so normal tools like `git
log` on that branch will never surface it, and it's eligible for
garbage collection eventually. "Gone from the branch" and "deleted
from disk" are different claims — the Scenario's teammate experiences
the first one, which is exactly as disruptive as it sounds even though
the second one hasn't necessarily happened yet.
