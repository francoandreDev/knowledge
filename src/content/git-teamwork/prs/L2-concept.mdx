...
structurally can and can't catch, and why blocking checks exist instead of
just asking people to run tests.

<LevelIntro
  items={[
    "Why passing tests isn't the same as correct",
    "What CI and review are each actually good at",
    "Why blocking checks exist instead of a request",
  ]}
/>
...
<Checkpoint question="A PR adds a new function and every existing test in the suite passes on it. Does that tell you the new function is correct, or only that it didn't break anything that was already being checked?">

Only the second one. A green CI run confirms the change didn't regress any
behavior the existing tests already exercise — it says nothing about whether
the new function itself behaves correctly on inputs nobody wrote a test for
yet. "Nothing broke" and "this is correct" are different claims, and CI can
only ever make the first one. Whether the new function is actually right is
exactly the kind of question that's still open after a fully green CI run,
which is why review exists as a separate, non-redundant step rather than a
second pass at the same thing CI just checked.
...
<Checkpoint question="A team configures a linter to run on every PR, but sets it to only post a warning comment rather than blocking the merge button. Based on the reasoning above about why blocking checks exist, what's likely to happen to that warning under deadline pressure?">

It's likely to get ignored, eventually, the same way an unenforced "please
run the tests" request eventually gets skipped. A non-blocking check still
depends on someone choosing to act on it in the moment, and under deadline
pressure "I'll clean this up later" is always available as a justification
— nothing actually stops the merge if they don't. The whole reason blocking
checks exist is to remove that choice; a check that can be overridden or
ignored will be, eventually, which is exactly why "visibility only" checks
tend to function very differently in practice from checks that are actually
