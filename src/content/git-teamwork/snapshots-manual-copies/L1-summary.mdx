...
<LevelIntro
  items={[
    "Why manual copies can't answer basic history questions",
    "The commit, branch, and distributed-history vocabulary",
    "The three-state model: working directory, staging, repository",
  ]}
/>

One important correction before the Git words start: Git does **not**
record every time you press Save in your editor. Saving changes the
file on disk. Git records a checkpoint only when you deliberately build
one with `git add` and `git commit`. That deliberate checkpoint is what
becomes comparable, nameable, and recoverable later.

| Human problem              | What a person would have to remember manually | What Git records instead                   |
| -------------------------- | --------------------------------------------- | ------------------------------------------ |
| Why did this file change?  | A note written somewhere else                 | The commit message attached to the save    |
| Which copy has both fixes? | Which folder was copied from which            | Parent links and merge commits             |
| When did the bug appear?   | Every old zip, opened and tested by hand      | Ordered commits you can inspect one by one |
...
- A **tracked** file is one Git already knows belongs in history. An
  **untracked** file is visible to Git, but you have not promised to
  include it in any commit yet. The `working-tree` unit goes deeper on
  this three-area model.
...
Git can merge two histories because both branches remember the commit
they started from. That shared starting point lets Git ask, "what did
Priya change from the base, and what did Alex change from the same
base?" The `merge` unit teaches that mechanism; here the important
point is that manual folders do not preserve that relationship.
