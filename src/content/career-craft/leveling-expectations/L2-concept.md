...
```python
function assess_level(person):
    skill = technical_ability(person)              # can they solve hard problems, given a defined problem?
    scope = largest_ambiguity_resolved_unprompted(person)  # do they FIND the problems, unassigned?

    # These are correlated (skill enables handling wider scope) but not
    # the same axis — a highly skilled person kept on narrowly-scoped
    # tasks doesn't automatically read as operating at wide scope, and
    # someone still growing technically can be the one who noticed and
    # started fixing a cross-team problem nobody assigned them.
    return combine(skill, scope)  # leveling conversations are really about scope
```
