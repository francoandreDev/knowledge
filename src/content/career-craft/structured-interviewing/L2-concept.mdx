...
L1 named the halo effect and drew the line between an unstructured
interview and a structured one. This level goes one layer deeper: why
fixed questions and a predefined rubric are specifically what breaks the
halo effect's grip, rather than just "more effort" or "trying to be fair."

<LevelIntro
  items={[
    "Why intent alone can't fix the halo effect",
    "What a real rubric anchor requires",
...
<Checkpoint question="An interviewer sincerely believes they are being fair to both candidates. Does that belief mean the halo effect isn't affecting their scores?">

No. The halo effect operates below the level of conscious intent — it
shapes which follow-up questions feel natural to ask and how an ambiguous
answer gets interpreted, not through a deliberate choice to favor someone.
An interviewer who sincerely believes they're being fair can still be
acting on an early impression; sincerity only means they aren't aware of
it happening, not that it isn't happening. That's exactly why the fix has
to be structural (fixed questions, a predefined rubric) rather than just
asking interviewers to try harder — trying harder doesn't touch a
mechanism the interviewer can't see themselves using.
...
<Checkpoint question="A rubric says '1-5, rate problem-solving ability' with no further detail. A candidate gives a confident, articulate answer with no real structure or trade-offs. Would this rubric reliably stop that confidence from inflating the score?">

No. Without anchors spelling out what a 1, 3, or 5 concretely requires,
the interviewer still has to decide in the moment what "good
problem-solving" looks like — and that in-the-moment judgment is exactly
where the halo effect re-enters, now hiding behind the appearance of a
numeric, "objective" scale. A confident, articulate delivery can easily
read as a higher number even though the anchor table above would score
that same answer a 1 or 3 for lacking a trade-off or validation step. The
scale only does its job once each number is tied to concrete, predecided
content — not before.
