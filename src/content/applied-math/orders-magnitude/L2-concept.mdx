...
L1 named the incident's actual error — a KB figure got labeled MB, inflating a 10 GB
requirement into a reported 10 TB — and gave the two skills that would have caught it:
tracking units algebraically, and cross-checking with a rough estimate. L2 works out both
of those mechanically, precisely enough to run by hand on any calculation, not just this
one.

<LevelIntro
  items={[
    "Canceling units like algebraic terms",
    "Unit conversion as multiplying by 1",
    "Fermi estimation for a fast sanity check",
  ]}
/>
...
<Checkpoint question="A calculation multiplies 3 servers by 4 requests/second/server. Cancel the units by hand the way the KB/record example did above — what unit is the result in, and why?">

`requests/second` — "server" appears once as the thing being counted (3 servers) and once
as "per server" inside the rate, so those two labels cancel exactly like `records` and
`record` did above, leaving `requests/second` as the derived unit. Nothing about the
numbers (3 and 4) decides the unit; only which labels had a matching opposite to cancel
against does.
...
<Checkpoint question="Why does multiplying by `1 MB / 1,000 KB` change the written number but not the actual quantity of data?">

Because that fraction equals exactly 1 — 1 MB and 1,000 KB are two names for the same
amount of data, so multiplying by it is multiplying by 1 in disguise, same as multiplying
any number by `5/5`. The quantity of data never changes; only which unit is used to
express it does. That's also exactly why _skipping_ the conversion is dangerous: the
written number stays `10,000,000` either way, but without the conversion it's still
attached to the wrong unit label, so the reader thinks the quantity is 1,000x larger than
it actually is.
