...
<LevelIntro
  items={[
    "Reproducing the Goodhart curve in code",
    "Simulating sampling error directly",
...
<Checkpoint question="Two teams both apply the same optimization pressure to their proxy. Team A's proxy has corr=0.9, Team B's has corr=0.3. Both proxy scores hit the same number at pressure=100. Does that shared proxy score tell you their real targets are also close together?">

No. `proxyScore` doesn't depend on `correlation` at all — it always climbs the same way with pressure, capping at 100 regardless of which team you're looking at. The `penalty` term in `realTarget` is what depends on correlation, and it grows with the square of pressure for a low-correlation proxy, so at high pressure the two real targets can be far apart (90 vs. 30) even while their proxy scores are identical. A proxy score alone never tells you how well it's tracking the real target — you need to know the correlation separately, and that's exactly the thing a dashboard showing only the proxy number can't show you.
...
<Checkpoint question="You're sampling 1% of production requests and want to cut sampling error roughly in half without changing anything else about how you sample. Does doubling the sample rate to 2% get you there?">

No — cutting sampling error in half needs roughly 4x the sample size, not 2x, because the error shrinks proportional to `1 / √n`. Doubling from 1% to 2% only shrinks the error by a factor of about `1/√2` (≈0.71), not `1/2`. To actually halve the error you'd need to go from 1% to 4%. This is also why the last stretch of sampling (going from a partial sample to 100%, i.e. dropping sampling entirely) buys comparatively little additional accuracy for a large jump in instrumentation cost — the `1/√n` curve flattens out.
