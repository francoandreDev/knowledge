...
L1 showed Team A and Team B driving the same proxy — test coverage — toward the same number while their real targets went in opposite directions. This level builds the model that explains why that's possible at all.

<LevelIntro
  items={[
    "The proxy → instrument → number chain",
    "Reliable-but-invalid measurements",
    "Why optimizing a proxy can break it (Goodhart)",
  ]}
/>
...
<Checkpoint question="A dashboard shows the exact same p99 latency, down to the millisecond, every time you check it this week — suspiciously stable. Does that stability tell you the number is accurate?">

No. Stability tells you the measurement is _reliable_ (consistent), not that it's _valid_ (correct). A dashboard that always shows the same value could be reliably reading a miscalibrated instrument, reliably sampling only a biased subset of traffic (e.g. only business-hours requests), or even stuck returning a cached/stale value. Reliability and validity are separate properties that have to be checked separately — this is the same trap as the 5-lbs-heavy scale: it never contradicts itself, which is exactly what makes it easy to trust by mistake.
