...
L1 showed that 4 flags produce 16 states, not 8 or 4, and named the
building-block terms — factorial, permutation, combination. This level
works through why that multiplication happens and how to tell a
permutation problem apart from a combination problem.

<LevelIntro
  items={[
    "Why flags double instead of adding",
    "Permutations vs. combinations formulas",
    "Counting flag pairs with nC2",
  ]}
/>
...
<Checkpoint question="A team adds a 5th independent flag to the checkout feature, bringing the total from 4 flags to 5. How many total on/off states does that create, and is it a small bump or a big jump from 16?">

32 states — double the 16 that 4 flags produced, not 16 + 1 or 16 + 5.
Every one of the 16 existing states can now occur with the new flag
either on or off, so the new flag doesn't add a fixed number of new
situations — it doubles every situation that already existed. That's
the exponential-growth mechanism from this section, not a one-off
coincidence: each additional independent flag will keep doubling the
count from here on (64, 128, 256...), which is why "add a few more
test cases to cover the new flag" stops being a workable strategy long
before the flag count gets large.
...
<Checkpoint question="Two candidates, Alice and Bob, both end up on the picked team of 3 from 5. Does it matter, for the combination count, whether Alice or Bob was technically 'chosen first'?">

No — and that's the entire difference from a permutation. A
combination only cares about which 3 people end up on the team, not
the order they were selected in, so "Alice then Bob" and "Bob then
Alice" collapse into the same outcome. That's exactly why the `nCr`
formula divides the permutation count by `r!`: it's removing the
reorderings of the chosen group that a permutation would have counted
as distinct. This distinction is what the flag-pairs question below
depends on — counting flag pairs only works because a pair like
`applePay, promoCode` is the same pair regardless of which flag is
named first.
