...
A **stakeholder** is someone affected by the result or responsible for
asking for it. In this story, Elena is the stakeholder because her team
will use the workflow and live with the consequences.

</Scenario>

<LevelIntro
  items={[
    "Three ways the same ticket can be handled",
    "Where each approach's gaps get found, and what that costs",
    "Common failure modes in requirements gathering",
  ]}
/>
...
**Compliance** means rules the team must follow, sometimes legal and
sometimes internal. In this example, the rule is simple: every status
change must leave an audit log entry, whether it is done one account at a
time or in bulk.

<Checkpoint question="Request 2 got Sam the right operation (status changes, not deletion) — a real improvement over Request 1. So why did it still ship with a gap that took a compliance review to catch?">

Because Sam stopped asking after the first answer resolved the most
obvious ambiguity, and treated that as the end of the conversation instead
of a foothold for more of it. Elena answered exactly what she was asked —
what workflow the ticket would replace — but she never volunteered the
audit-log requirement, because from her side it wasn't a new fact worth
mentioning; it's just how status changes already work, so it didn't occur
to her to restate it for the bulk version. The gap wasn't in Elena's
answer, it was in the conversation stopping one round too early: getting
the core operation right doesn't confirm there's nothing else riding along
with it, like an existing norm nobody thought to name out loud.
...
Here, **v1** means "the first version we are promising." It can improve
later, but the acceptance criteria above define what this first delivery
...
operation, the allowed statuses, the scale, and the compliance requirement
— not word count, which doesn't track this at all):
...
These are common ways a requirement can look clear while still hiding a
missing fact.
...
<Checkpoint question="One team writes an acceptance criterion as 'use a Set to track selected account IDs so duplicates can't be added twice.' Is this a good acceptance criterion? What test would you apply to decide?">

No — it fails the same falsifiability test from L2, just in a different
disguise. A good acceptance criterion has to be checkable by someone who
never looks at the code, only at the built thing's behavior: can a
reviewer select the same account twice, does that cause a problem, yes or
no. "Use a Set" describes a specific way to implement that guarantee, not
the guarantee itself, and it locks the team into one implementation choice
for no reason the requirement actually needs. The fix is to write what the
behavior must be — "selecting an already-selected account has no effect,
it isn't added twice" — and leave how that gets built entirely up to
whoever implements it, the same way the rest of this unit's acceptance
criteria describe outcomes, not code.
