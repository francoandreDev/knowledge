...
- **The five whys** is a simple way to keep pushing past a stated request to the actual underlying need: "I want a CSV export" → why? → "So I can build a report" → why? → "So I can see weekly trends" → why? → "So I know if the metric is getting worse" → the real need is _trend visibility_, and CSV export is just the first solution Mara happened to imagine.
