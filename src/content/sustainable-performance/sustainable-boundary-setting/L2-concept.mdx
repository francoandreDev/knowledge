---
title: "L2 — Why trade-off framing lands differently than a flat no, and what makes a capacity signal credible"
---

import LevelIntro from "../../../components/LevelIntro.astro";
import Checkpoint from "../../../components/Checkpoint.astro";

L1 named the risk in a flat "no" — that it reads as a capability
judgment instead of the capacity statement it's usually meant to be.
L2 works out why trade-off framing avoids that trap, and what
actually makes a capacity signal land as credible information rather
than a vague excuse.

<LevelIntro
  items={[
    "Why a flat no gets misread",
    "Trade-off framing and who should decide",
    "What makes a capacity signal credible",
  ]}
/>
...
<Checkpoint question="A teammate asks a colleague to review their pull request 'today if possible.' The colleague replies, 'No, I can't.' Using the distinction above, what is that reply likely to be heard as, and what's missing from it that would change that?">

It's likely to be heard as reluctance or an unexplained refusal, because
it gives the teammate nothing to weigh it against — no sense of whether
the colleague is genuinely swamped, doesn't think the request matters, or
just doesn't want to do it. What's missing is the capacity context: naming
what's already committed (say, two reviews due before lunch) turns the
same refusal into a factual statement about current load, which reads as
informed prioritization rather than a judgment call about willingness.
...
```mermaid
flowchart LR
    A["New request arrives"] --> B["State current commitments\n(the capacity signal)"]
    B --> C["Offer the trade-off:\n'I can take this on if X moves,\nor decline and keep X on track'"]
    C --> D["Requester decides,\nwith the real cost visible"]
```
...
<Checkpoint question="Two people decline the same request. One says 'I'm swamped right now.' The other says 'I'm finishing the Atlas rollout and the billing fix, both due Friday.' Which is more likely to actually change what the requester does next, and why?">

The second one, because it hands the requester something concrete to act
on. "I'm swamped" can't be weighed against anything — the requester has
no way to judge whether the new request is more or less urgent than
whatever "swamped" refers to, so it tends to get either dismissed or
repeated as pressure. Naming the specific commitments and their deadlines
gives the requester an actual basis for a decision — they can compare the
new request's urgency against Friday's deadlines and decide what should
move, which is exactly the kind of decision a vague claim can't support.
