...
<LevelIntro
  items={[
    "Two paths from the same early signs",
    "What made Path B's response possible",
    "Failure modes in applying this in practice",
  ]}
/>
...
<Checkpoint question="Path B's manager conversation named 'exhaustion AND increasingly cynical, and that specific combination' rather than just saying 'I'm burned out.' Why does the more specific phrasing matter, given how the two paths played out?">

Because a vague claim like "I'm burned out" invites an equally vague, dismissible response — "everyone's tired, push through" — since it sounds interchangeable with ordinary tiredness and gives the listener nothing concrete to evaluate. Naming the specific combination of exhaustion and cynicism does two things a vague claim can't: it shows the person has actually checked their own state against real criteria rather than just venting, and it gives the manager something specific enough that "everyone's tired" doesn't actually address it. That precision is also what let Path B catch the pattern by month 3 with only two of three components present, instead of waiting until it became undeniable the way Path A's vaguer, individually-dismissed signs did.
