---
title: "L2 — Why an interruption costs more than the interruption itself"
---

import LevelIntro from "../../../components/LevelIntro.astro";
import Checkpoint from "../../../components/Checkpoint.astro";

L1 named the gap between shallow work and deep work, and pointed at
attention residue as the reason an interruption costs more than its
own length. L2 works out exactly why that's true, and what it implies
for how a day should actually be organized.

<LevelIntro
  items={[
    "The true cost of an interruption",
    "Why frequency matters more than length",
    "Protecting blocks instead of reacting all day",
  ]}
/>
...
```mermaid
flowchart LR
    A["Deep work:\nbuilding up context"] --> B["Interruption:\n~30 sec to answer"]
    B --> C["Attention residue:\n~15 min refocusing"]
    C --> D["Back to full depth"]
```
...
<Checkpoint question="A teammate says 'I only spent 11 minutes on Slack today, so it barely affected my work.' Using the distinction between direct handling and attention residue, what's wrong with that reasoning?">

The 11 minutes only counts the direct handling — the actual seconds
spent reading and typing each reply. It leaves out the much larger
cost: the time spent afterward rebuilding the depth that was lost each
time a reply pulled attention away from the hard task. A teammate can
be completely honest about the 11 minutes and still be wrong about
where the day went, because the number that actually explains a
mostly-empty day is the refocusing tax, not the handling time. Judging
an interruption's cost by how long it took to answer is exactly the
mistake this section is warning against.
...
<Checkpoint question="A manager insists that protecting a deep-work block means the amount of time spent on shallow work has to go down. Is that what the fix actually requires?">

No. The fix reorganizes _when_ shallow work happens, not _how much_ of
it happens. All the messages, unblocking, and status checks still get
answered — they move into a dedicated batched window instead of being
scattered across the whole day. The total minutes spent on shallow
work can stay exactly the same; what changes is that none of those
minutes land inside a block that was supposed to stay protected, so
the deep-work block never pays the attention-residue tax in the first
place.
