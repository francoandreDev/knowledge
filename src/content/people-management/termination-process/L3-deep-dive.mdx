---
title: "L3 — The Scenario's actual meeting, opening line to close"
---

import LevelIntro from "../../../components/LevelIntro.astro";
import Checkpoint from "../../../components/Checkpoint.astro";

L2 established the four-step structure and why the reason has to stay
tied to the documented PIP. L3 walks the Scenario's manager through
the real conversation, line by line, including the moment the person
pushes back.

<LevelIntro
  items={[
    "What has to be ready before walking in",
    "The opening line and reason, word for word",
    "Handling pushback without relitigating",
  ]}
/>
...
<Checkpoint question="Of the four things in the table above, which one exists specifically so the manager isn't the only person accountable for how the conversation goes, rather than being information the manager needs to state?">

The second person in the room. The other three rows are facts the
manager needs to have ready to state accurately; the second person
isn't a fact at all, it's a structural safeguard — someone who can
witness the conversation, catch a moment where the manager starts to
over-explain or improvise under pressure, and field questions (benefits
specifics, legal details) that aren't the manager's to answer. It's
the one preparation item that protects the process itself, not just
...
```mermaid
xychart-beta
    title "Words spoken about 'why', by conversation stage"
    x-axis ["Opening reason (1 sentence)", "If pushback: restated once", "If pushback again: still not re-argued"]
    y-axis "New justifications introduced" 0 --> 3
    bar [0, 0, 0]
```
