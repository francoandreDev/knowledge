...
L1 named the SBI structure and showed a real example of trait-based
feedback landing with no effect. L2 works out precisely why swapping a
trait for Situation-Behavior-Impact changes what the receiver can act on,
and how delivery timing interacts with that structure.

<LevelIntro
  items={[
    "Why trait labels give nothing to act on",
    "SBI versus trait-based, side by side",
    "How timing and repetition change the conversation",
  ]}
/>
...
<Checkpoint question="A manager says 'you did great work this sprint.' Is that SBI feedback, or trait-based feedback wearing a positive tone instead of a critical one?">

Trait-based. "Great work" names a conclusion, not a specific situation, an observable behavior, or a concrete impact — it's just as vague as "be more proactive," it just feels nicer to hear. The receiver still can't tell which specific thing to keep doing. Praise needs the same S-B-I structure as correction to actually reinforce a repeatable behavior: naming the specific moment, what was done, and what it caused works the same way for positive feedback as for a difficult conversation.
...
<Checkpoint question="A manager delivers SBI-structured feedback three weeks after the event, from memory, without notes. Which letter of SBI is most likely to have quietly degraded back into a vague trait statement, and why that one specifically?">

The Situation. Behavior and Impact can sometimes still be recalled roughly right from memory, but the Situation is the most specific, single-instance detail — an exact moment, an exact exchange — and it's the first thing that blurs after three weeks. Once the manager can't cite the precise situation, they tend to fall back on "this happens a lot" or "you're often late," which is exactly the pattern-level trait language SBI was built to replace. Fresh delivery protects the Situation; a stale one erodes it first.
