...
L2 laid out the sequence in the abstract and the specificity test a
PIP's goals have to pass. L3 runs the Scenario's exact manager and
report through it end to end — the repair conversation, the
documentation trail made concrete, the written PIP itself, and both
ways the final checkpoint can go.

<LevelIntro
  items={[
    "The repair conversation that restores the missing warning step",
    "A concrete documentation trail and a full written PIP",
    "Both outcomes at the final checkpoint, and a genuine dispute case",
  ]}
/>
...
<Checkpoint question="The trail includes the manager's own Apr 2 corrective step (clarifying requirements in writing), not just the report's missed deadlines. Why does that inclusion matter to whether the eventual PIP is defensible?">

It matters because a one-sided record — only the report's failures,
none of the manager's own actions — reads as building a case rather
than documenting a genuine attempt to help someone improve. By
recording that the manager clarified requirements on Apr 2, the trail
answers the report's Mar 7 explanation directly: it shows the excuse
was taken seriously and addressed, so a similar miss after that date
can no longer be attributed to the same cause. That's what turns the
trail from a list of grievances into evidence that the process was
fair to both sides, which is exactly what makes the written PIP that
follows defensible instead of retaliatory-looking.
...
**What would change about this whole process if the report were a
brand-new hire still in their first 90 days, rather than someone with
years of solid history before this slump?** Consider it before moving
on: a short tenure changes how much benefit of the doubt "unclear
requirements" deserves, how much documentation history even exists to
draw on, and whether a company's own probationary-period policy
overrides the standard PIP sequence entirely. The Scenario's manager
and report are one case, not the whole territory this unit covers.
