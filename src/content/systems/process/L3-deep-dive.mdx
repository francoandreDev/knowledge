...
<LevelIntro
  items={[
    "Proving process isolation and copy-on-write with real output",
    "Proving the stack and heap are real, finite regions",
    "Reading a live process's memory map from the kernel",
  ]}
/>
...
<Checkpoint question="fork_isolation.py's child sets counter = 999 and the parent's counter stays 0. If copy-on-write means the pages start out shared, why doesn't the parent see 999 too, at least briefly, before any copying happens?">

Because "shared" here means shared physical pages, not a shared variable — the two processes still have separate page tables and separate virtual-to-physical mappings from the instant `fork()` returns; they just happen to point at the same physical page as a memory-saving optimization, since most forked processes don't immediately write to most of their memory. The moment the child executes `counter = 999`, the kernel intercepts that write (the shared page is marked read-only specifically to trigger this), copies the page so the child gets its own physical copy, and only then lets the write proceed — entirely inside the child's private mapping. The parent's page table was never touched, so its `counter` was never anything but `0`. Sharing physical pages is purely an implementation detail for efficiency; it never implies a shared variable that either side could observe the other writing to.
...
<Checkpoint question="stack.mjs throws a catchable RangeError; segfault.c gets killed by SIGSEGV with no chance to catch anything. Both are the same underlying limit being hit. What is actually different between the two, mechanically?">

What differs is who is enforcing the limit and when. In `stack.mjs`, the JavaScript runtime tracks its own call-stack depth in software and throws a normal, catchable exception once it crosses a threshold set comfortably below the process's real stack region — the process itself never actually runs out of stack memory. In `segfault.c`, there is no such software accounting; the C program keeps pushing real stack frames until the stack pointer moves past the last page the kernel mapped for that region, and only then does the hardware MMU fault and the kernel step in with `SIGSEGV`. So the JS case is a voluntary, early, graceful stop chosen by the runtime; the C case is the actual hardware/kernel boundary being hit for real, with no opportunity for the program to intervene. Same physical limit, but one is a runtime's polite simulation of it and the other is the limit itself.
