...
makes that mechanism directly measurable with real code: a naive version, a
fixed version, and a worked denormalization trade-off.

<LevelIntro
  items={[
    "Measuring the N+1 pattern with a real query counter",
    "Fixing it by fetching related data in one pass",
    "A worked denormalization trade-off, with real code on both sides",
  ]}
/>
...
<Checkpoint question="In the code above, if the Scenario's order list grew from 50,000 to 200,000 orders, what would queryCount be after calling listOrdersWithCustomerNaive, and why?">

200,001 — one call to `findAllOrders`, plus exactly one call to
`findCustomerById` per order, and the number of orders is what changed.
`queryCount` isn't tied to some fixed overhead of the function; it's tied
directly to `orders.length`, because the `.map()` callback calls
`findCustomerById` once per iteration. That one-to-one relationship between
list size and query count is the N+1 pattern itself, not just a symptom of
it — it's exactly why the endpoint that felt instant at 20 orders takes
nearly a minute at 50,000, and would take roughly four times as long again
at 200,000.
