...
L1 named the growth-rate classes and why a small test file can't reveal them. This level
goes one level deeper: how to read `O(n²)` off a real function that doesn't look nested,
and how to fix it by changing the data structure rather than adding cleverness to the loop.

<LevelIntro
  items={[
    "Spotting a hidden nested loop in real code",
    "Why a list scan and a hash lookup cost different amounts",
    "The mechanical rules for deriving Big-O by hand",
  ]}
/>
...
<Checkpoint question="The code above has exactly one `for` loop, not two. Why is it still O(n²) instead of O(n)?">

Because the membership check `record.id not in result` isn't the O(1) operation it looks
like on the page — `result` is a plain list, so checking whether an id is already present
means scanning every item currently in `result`, up to `n` of them, before answering.
That scan is a loop hiding inside a single line of code. The outer `for` loop already runs
`n` times on its own; multiplying that by an inner scan that can also run up to `n` times
gives `n × n = n²` total work, even though only one `for` keyword appears in the source.
The lesson generalizes past this one function: any operation whose own cost depends on how
much data has already accumulated — not just a literal second loop — can produce the same
quadratic shape.
...
<Checkpoint question="A teammate says 'I swapped the array for a Set, so this function is now guaranteed O(n), no matter what.' What's wrong with that claim?">

It overstates what the Set buys you. Swapping the array for a Set does take the _typical_
case from O(n²) down to O(n) — that part is correct — but the O(1) membership check a Set
provides is an average-case property, not a worst-case guarantee. If enough of the incoming
ids happen to collide in the Set's underlying hash buckets, individual lookups can degrade
back toward O(n), which would push the whole function back toward O(n²) in that pathological
scenario. So the honest claim is "O(n) on average, for ordinary data" — not "guaranteed O(n)
no matter what." That distinction matters most for judgment calls like whether a Set is
still safe to use with adversarial or attacker-controlled input, where a worst case has a
real chance of being hit rather than staying a theoretical footnote.
