...
- **Persistence**: IndexedDB (via a small promise wrapper, e.g. `idb`) for all game-owned state — tokens, coin balance, permanent upgrade levels, best time. The game _reads_ (never writes) the existing `localStorage` progress keys (`progress:<track>/<unit-slug>`) to compute tokens and run duration; it does not interfere with the site's existing `progress:`/`exercise:` localStorage keys.
  - **Implementation note (Phase 4):** the IndexedDB wrapper is Phase 5 work (alongside coins/shop). Until then, tokens and best-time live in `pStorage` (`src/lib/game/tokens.ts`) — the same profile-namespaced `localStorage` wrapper `progress:`/`exercise:` keys already use, since they're equally per-learner data.
  - **Implementation note (Phase 5):** the `idb` migration was deliberately skipped, not deferred further. `getTokenCount()`/`computeRunDurationS()` are read synchronously on every page load by `Layout.astro`'s nav badge — a shared component rendered site-wide, not just on `/game/`. Converting that to `idb`'s async API would mean either a flash-of-stale-badge on every navigation (read starts async, paints stale/zero first) or blocking every page's first render on an IndexedDB open, for a feature whose actual requirement (per-profile isolation, survives reload) `pStorage` already satisfies. `pStorage` also gets `deleteProfile()`'s cleanup for free (it sweeps every `p:<id>:*` key), whereas a separate IndexedDB database per profile would need its own explicit deletion wired into `deleteProfile()`. Coins and the 7 shop upgrade levels (`src/lib/game/shop.ts`) followed tokens/best-time into `pStorage` for the same reasons. Revisit only if a real async-safe need shows up (e.g. syncing across devices) — not preemptively.
